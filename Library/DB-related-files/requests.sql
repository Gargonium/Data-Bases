...
where r.category_id = :category_id -- 1
  ...
  and rcav.value = :attribute_value;
...
from readers r
         join loans l on r.id = l.reader_id
         join publications_copy pc on l.copy_inventory_number = pc.inventory_number
where pc.publication_id = :publication_id -- 2
  and l.return_date is null;
...
where r.id = :reader_id -- 1
  and r.library_id = (select library_id
                      from readers
                      where id = :reader_id)
  and l.date_of_issue between :date_from and :date_to;
...
select e.id, e.first_name, e.last_name, count(l.reader_id) as readers_served
from employee e
...
where library_id = :library_id -- 2
  and reading_room_id = :reading_room_id;
-- 5
...
where w.author = :author_id;
-- 5


-- 16. Получить список самых популярных произведений.
select w.id, w.title, count(l.id) as usage_count
from works w
         join publications_works pw on w.id = pw.work_id
         join publications_copy pc on pw.publication_id = pc.publication_id
         join loans l on pc.inventory_number = l.copy_inventory_number
group by w.id, w.title
having count(l.id) >= (select count(l2.id)
                       from works w2
                                join publications_works pw2 on w2.id = pw2.work_id
                                join publications_copy pc2 on pw2.publication_id = pc2.publication_id
                                join loans l2 on pc2.inventory_number = l2.copy_inventory_number
                       group by w2.id
